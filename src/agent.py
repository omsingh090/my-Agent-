...
class BusinessDecisionAgent:
    ...
    1. Cleaned datasets
    2. Meaningful analytical insights
    3. Actionable business decisions
    4. Prioritized recommendations
    
    Pipeline: Load → Clean → Analyze → Decide → Evaluate
    """
    ...
    def run(self, file_path: str) -> Dict[str, Any]:
        ...
        Example:
            ...
            >>> results = agent.run("data/sales.csv")
        ...
        try:
            ...
            metrics = self._evaluate_performance(analysis, decisions)
    ...
    def _analyze_data(self, df):
        ...
        logger.info(f"✓ Anomalies detected: {sum(len(a.get('indices', [])) for a in analysis['anomalies'].values())}")
        return analysis

    def _generate_decisions(self, analysis):
        """Generate and rank decisions."""
    ...
    def _evaluate_performance(self, analysis, decisions):
        """Evaluate agent performance."""
        processing_time = time.time() - self.start_time

        # Extract metrics
        data_quality = self.results["clean_report"].get("quality_score", 80)
        insight_count = len(analysis.get("trends", [])) + len(analysis.get("correlations", []))
        decision_count = len(decisions)
        avg_priority = sum(d.get("priority_score", 5) for d in decisions) / max(decision_count, 1)
    ...
    def _print_summary(self):
        ...
        for i, decision in enumerate(decisions[:3], 1):
            ...
            logger.info(f"   📊 Impact: {decision['impact']}/10 | ⚙️ Effort: {decision['effort']}/10 | 💰 Value: {decision['business_value']}/10")
            
            if decision['do_this_first']:
                logger.info(f"   >>> DO THIS FIRST <<<")
    ...
    def export_json(self, output_path: str = "output.json") -> str:
    ...
    def get_ceo_summary(self) -> str:
        """
        Get one-page executive summary.
        ...
        summary = []
        summary.append("AI BUSINESS DECISION ANALYST - EXECUTIVE SUMMARY")
        summary.append("=" * 50)
        summary.append("")

        # Top 3 decisions
        summary.append("TOP 3 DECISIONS (Ranked by Priority)")
        summary.append("-" * 50)
        for i, decision in enumerate(decisions[:3], 1):
            summary.append(f"\n{i}. {decision['title']}")
            summary.append(f"   Priority: {decision['priority_score']}/10")
            summary.append(f"   Evidence: {decision['evidence']}")

        # Performance
        summary.append("\n\nPERFORMANCE GRADE")
        summary.append("-" * 50)
        summary.append(f"Overall Score: {metrics['total']:.0f}/{metrics['max']}")
        summary.append(f"Grade: {metrics['grade']}")
